      ...
      <CardContent className="pt-5">
        {status === null ? (
          ...
          <div className="space-y-4">
            ...
            {/* Key is set but Gemini rejected the test call - show why */}
            {status.configured && !status.connected && (
              <div className="rounded-2xl border border-rose-200/60 bg-rose-50/60 px-4 py-3.5 text-sm text-rose-800">
                <p className="font-medium mb-1">Gemini test call failed</p>
                <p className="text-rose-700/80 leading-relaxed">
                  {status.message || "Check GEMINI_API_KEY in the backend .env file and restart the server."}
                </p>
              </div>
            )}
